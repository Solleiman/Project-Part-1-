...
class OutputInventory:
    ...
    def damaged(self):
        ...
        - items sorted by price from most to least expensive
        item info:
        Item ID, Manufacturer name, item type, price, service date
        '''
        items = self.item_list
        # get order of keys based off price to write into the file
        ...
        with open('./output_files/DamagedInventory.csv', 'w') as file:
            for item in keys:
                ...
                item_type = items[item]['item_type']
                price = items[item]['price']
                service_date = items[item]['damaged']
                if damaged:
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        #Get diff manufacturers and types
        #put in a list
        types = []
        manufacturers = []
        for item in items:
        ...
        while user_input != 'q':
            ...
            if user_input == 'q':
                ...
                #check each word to see for match in manufacturer and item type
                ...
                for word in user_input:
                    if word in manufacturers:
                        if selected_manufacturer:
                            #should have only one manufacturer
                            bad_input = True
                        else:
                ...
                if not selected_manufacturer or not selected_type or bad_input:
                    ...
                    #order list of keys to iterate based on highest price first
                    keys = sorted(items.keys(), key=lambda x: items[x]['price'], reverse=True)

                    #get matching list of items
                    matching_items =[]
                    #Store items with same type but different manufacturer
                    # item type matched
                    # item must be in inventory, not past service, and not damaged
                    similar_items = {}
                    for item in keys:
                        if items[item]['item_type'] == selected_type:
                            #dont add damaged items or past service to matched list or similar list
                            today = datetime.now().date()
                            service_date = items[item]['service_date']
                            service_expiration = datetime.strptime(service_date, "%m/%d/%Y").date()
                            expired = service_expiration < today
                    ...
                    #Output item from diff manufacturer that is similar in price to matched item
                    if similar_items:
                        matched_price = price
                        #Get similar item
                        closest_item = None
